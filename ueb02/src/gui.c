...
static void gui_widgetVec3(struct nk_context* nk, vec3 val)
{
	...
	nk_property_float(nk, "#X:", -100.0f, &nkTrans.x, 100.0f, 1.0f, 0.1f);
	nk_property_float(nk, "#Y:", -100.0f, &nkTrans.y, 100.0f, 1.0f, 0.1f);
	nk_property_float(nk, "#Z:", -100.0f, &nkTrans.w, 100.0f, 1.0f, 0.1f);
...
static void gui_widgetLightComp(struct nk_context* nk, const char* name, vec4 col)
{
	...
	{
		//nk_layout_row_dynamic(nk, 120, 1);
		//nkLight = nk_color_picker(nk, nkLight, NK_RGBA);
		nk_layout_row_dynamic(nk, 25, 1);
		nkLight.r = nk_propertyf(nk, "#Ambient:", 0, nkLight.r, 2.0f, 0.01f, 0.005f);
		nkLight.g = nk_propertyf(nk, "#Spekular:", 0, nkLight.g, 2.0f, 0.01f, 0.005f);
		nkLight.b = nk_propertyf(nk, "#Diffuse:", 0, nkLight.b, 2.0f, 0.01f, 0.005f);
		nk_combo_end(nk);
	}
...
static void gui_renderMenu(ProgContext* ctx, struct nk_context* nk)
{
	...
	if (input->showMenu)
	{
			...
			NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
		{
			...
			// Einstellungen bezüglich der Darstellung.
			if (nk_tree_push(nk, NK_TREE_TAB, "Punktlicht Transformation", NK_MINIMIZED))
			{
				// Rotation de-/aktivieren
				nk_bool rotation = input->showRotation;
				if (nk_checkbox_label(nk, "Rotation:", &rotation));
				{
					input->showRotation = rotation;
				}
				if (nk_tree_push(nk, NK_TREE_NODE, "Translation", NK_MINIMIZED))
				{
					gui_widgetVec3PointLight(nk, input->rendering.point_translate);

					nk_tree_pop(nk);
				}
				if (nk_tree_push(nk, NK_TREE_NODE, "Skalierung", NK_MINIMIZED))
				{
					gui_widgetVec3(nk, input->rendering.point_scale);
			...
			{
				enum choice { PHONG, GEONORMAL, CUBEMAP, TEXTUR, DIFFUSE, SPECULAR, EMISSION, NORMAL };

				static int s_choice = PHONG;
				s_choice = nk_option_label(nk, "Phong", s_choice == PHONG) ? PHONG : s_choice;
				s_choice = nk_option_label(nk, "Geometrie", s_choice == GEONORMAL) ? GEONORMAL : s_choice;
				s_choice = nk_option_label(nk, "Cubemap", s_choice == CUBEMAP) ? CUBEMAP : s_choice;
				s_choice = nk_option_label(nk, "Textur", s_choice == TEXTUR) ? TEXTUR : s_choice;
				s_choice = nk_option_label(nk, "Diffuse", s_choice == DIFFUSE) ? DIFFUSE : s_choice;
				s_choice = nk_option_label(nk, "Specular", s_choice == SPECULAR) ? SPECULAR : s_choice;
				s_choice = nk_option_label(nk, "Emission", s_choice == EMISSION) ? EMISSION : s_choice;
				s_choice = nk_option_label(nk, "Normal", s_choice == NORMAL) ? NORMAL : s_choice;

				input->shaderChoice = s_choice;
				nk_tree_pop(nk);
			}
			if (nk_tree_push(nk, NK_TREE_TAB, "Nebel", NK_MINIMIZED))
			{
				static float s_floatSlider = 0.0f;
				if (nk_button_label(nk, input->showFog ? "Nebel aus" : "Nebel an"))
				{
					input->showFog = !input->showFog;
				}

				nk_label(nk, "Nebel Dichte", NK_TEXT_LEFT);
				if (nk_slider_float(nk, 0.0f, &s_floatSlider, 1.0f, 0.01f))
				{
					input->density = s_floatSlider;
				}

				// Clear Color
				gui_widgetColor(nk, "Nebel Farbe:", input->rendering.clearColor);
