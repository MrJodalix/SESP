...
/**
 * Callback Funktion, die bei einer Größenberänderung des Framebuffers
 * aufgerufen wird.
 * 
    ...
    if (ctx->input->isFullscreen != (glfwGetWindowMonitor(ctx->window) != NULL))
    {
        ...
        else
        {
            ...
            glfwSetWindowMonitor(
                ...
                ctx->winData->cachedPosX, ctx->winData->cachedPosY,
                ctx->winData->cachedWidth, ctx->winData->cachedHeight,
                GLFW_DONT_CARE
            );
        }
