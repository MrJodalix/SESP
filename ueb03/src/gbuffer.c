...
GBuffer* gbuffer_createGBuffer(int width, int height)
{
    ...
    for (unsigned int i = 0; i < sizeof(gbuffer->textures) / sizeof(gbuffer->textures[0]); i++) {
        ...
        case GBUFFER_COLORATTACH_TEXCOORD:
            ...
            break;
        case GBUFFER_COLORATTACH_POSTPROCESS:
            common_labelObjectByType(GL_TEXTURE, gbuffer->textures[i], "POSTPROCESS");
            break;
        }
...
||||||| .r46
    // explicitly tell OpenGl which color attachments to be used (of. FB)
    // enable writing to all five textures
...
void gbuffer_bindGBufferForGeomPass(GBuffer *gbuffer)
{
    ...
    GLenum drawBuffer[] = {
        ...
        GL_COLOR_ATTACHMENT0 + GBUFFER_COLORATTACH_POSTPROCESS, // location 5
        GL_COLOR_ATTACHMENT0 + GBUFFER_COLORATTACH_FINAL, // location 6
    };
...
void gbuffer_bindGBufferForPostProcess(GBuffer* gbuffer)
{
    ...
    glReadBuffer(GL_COLOR_ATTACHMENT0 + GBUFFER_COLORATTACH_POSTPROCESS);
...
<<<<<<< .mine
    ...
    glDeleteTextures(1, &gbuffer->textures[GBUFFER_COLORATTACH_POSTPROCESS]);
||||||| .r46
=======
    glDeleteTextures(1, &gbuffer->textures[GBUFFER_COLORATTACH_TEXCOORD]);
>>>>>>> .r78
